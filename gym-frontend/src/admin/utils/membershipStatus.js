 ...
 * "expired" - "expiring" is derived here, not stored, matching the
 * same 7-day threshold used everywhere else this table is shown.
 */
export const getMembershipStatus = (membership) => {
    ...
    if (remaining !== null && remaining <= EXPIRING_SOON_THRESHOLD_DAYS) {
        return "expiring";
    }
...
export const getCurrentMembership = (id_user, memberships) => {
    ...
    const own = memberships.filter((m) => m.id_user === id_user);

    if (own.length === 0) {
        return null;
    }

    const active = own.find((m) => m.state === "active");
    ...
    return own.reduce((latest, m) =>
        new Date(m.end_date) > new Date(latest.end_date) ? m : latest
    );
